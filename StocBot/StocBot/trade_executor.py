...
class TradeExecutor:
    ...
    def execute_bracket_order(self, 
        ...
        try:
            ...
            if market == 'US':
                ...
                # Ensure take profit and stop loss prices are not None before calling the API
                # Default to reasonable values if not provided
                tp_price = take_profit_price
                sl_price = stop_loss_price
                
                # If take profit is not provided, set a default 5% above entry price for buy orders
                # or 5% below for sell orders
                if tp_price is None:
                    current_price = entry_price or self._get_current_price(symbol)
                    if current_price:
                        tp_price = current_price * 1.05 if side.lower() == 'buy' else current_price * 0.95
                    else:
                        logger.error(f"Cannot set default take profit: unable to get price for {symbol}")
                        raise ValueError(f"Take profit price is required for bracket orders")
                
                # If stop loss is not provided, set a default 2% below entry price for buy orders
                # or 2% above for sell orders
                if sl_price is None:
                    current_price = entry_price or self._get_current_price(symbol)
                    if current_price:
                        sl_price = current_price * 0.98 if side.lower() == 'buy' else current_price * 1.02
                    else:
                        logger.error(f"Cannot set default stop loss: unable to get price for {symbol}")
                        raise ValueError(f"Stop loss price is required for bracket orders")
    ...
    def get_position(self, symbol: str, market: str = 'US') -> Dict:
        ...
        try:
            ...
            # Default return if market is neither US nor India
            return {
                'success': False,
    ...
    def _get_current_price(self, symbol: str) -> Optional[float]:
        ...
        try:
            ...
            result = alpaca_api.get_last_price(symbol)
            if result.get('success', False):
