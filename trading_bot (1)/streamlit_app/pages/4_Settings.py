...
from utils.angel_one_api import test_angel_one_connection
...
if 'angel_one_api_key' not in st.session_state:
    st.session_state.angel_one_api_key = os.environ.get('ANGEL_ONE_API_KEY', '')
if 'angel_one_client_id' not in st.session_state:
    st.session_state.angel_one_client_id = os.environ.get('ANGEL_ONE_CLIENT_ID', '')
if 'angel_one_password' not in st.session_state:
    st.session_state.angel_one_password = os.environ.get('ANGEL_ONE_PASSWORD', '')
if 'angel_one_historical_api_key' not in st.session_state:
    st.session_state.angel_one_historical_api_key = os.environ.get('ANGEL_ONE_HISTORICAL_API_KEY', '')
if 'angel_one_market_feed_api_key' not in st.session_state:
    st.session_state.angel_one_market_feed_api_key = os.environ.get('ANGEL_ONE_MARKET_FEED_API_KEY', '')
...
with tabs[0]:  # API Connections
    ...
    # Angel One API Settings (Indian Stock Market)
    st.markdown('<p class="section-header">Angel One API Settings (Indian Stock Market)</p>', unsafe_allow_html=True)
    
    # Use a different style for Angel One settings to highlight it
    st.markdown("""
    <div style="padding: 10px; border-left: 3px solid #FF9800; margin-bottom: 20px; background-color: #FFF8E1;">
        Angel One API provides access to the Indian stock market, including NSE and BSE exchanges.
    </div>
    """, unsafe_allow_html=True)
    ...
    with col1:
        angel_one_api_key = st.text_input("Angel One API Key (Trading)", 
                                           value=st.session_state.angel_one_api_key, 
                                           type="password",
                                           help="API Key for Angel One trading operations")
        
        angel_one_client_id = st.text_input("Angel One Client ID", 
                                             value=st.session_state.angel_one_client_id,
                                             help="Your Angel One client/user ID")
        
        angel_one_password = st.text_input("Angel One Password", 
                                            value=st.session_state.angel_one_password, 
                                            type="password",
                                            help="Password for Angel One authentication")
    
    with col2:
        angel_one_historical_api_key = st.text_input("Angel One Historical API Key", 
                                                      value=st.session_state.angel_one_historical_api_key, 
                                                      type="password",
                                                      help="API Key for historical market data")
        
        angel_one_market_feed_api_key = st.text_input("Angel One Market Feed API Key", 
                                                       value=st.session_state.angel_one_market_feed_api_key, 
                                                       type="password",
                                                       help="API Key for real-time market feeds")
    ...
    with col1:
        if st.button("Test Angel One Connection"):
            with st.spinner("Testing connection to Angel One API..."):
                # Update session state first
                st.session_state.angel_one_api_key = angel_one_api_key
                st.session_state.angel_one_client_id = angel_one_client_id
                st.session_state.angel_one_password = angel_one_password
                st.session_state.angel_one_historical_api_key = angel_one_historical_api_key
                st.session_state.angel_one_market_feed_api_key = angel_one_market_feed_api_key
                
                success, message = test_angel_one_connection()
                
                if success:
                    st.success(message)
                    st.session_state.angel_one_connected = True
                else:
                    st.error(message)
                    st.session_state.angel_one_connected = False
    
    with col2:
        if st.button("Save Angel One Settings"):
            # Update session state
            st.session_state.angel_one_api_key = angel_one_api_key
            st.session_state.angel_one_client_id = angel_one_client_id
            st.session_state.angel_one_password = angel_one_password
            ...
            st.success("Angel One API settings saved successfully!")
...
with tabs[3]:  # Data Sources
    ...
    primary_data_source = st.selectbox(
        ...
        options=["Alpaca", "Angel One (Indian Market)", "Alpha Vantage", "Polygon.io", "Finnhub", "Yahoo Finance"],
        index=0
    )
    
    backup_data_source = st.selectbox(
        "Backup Market Data Source",
        options=["None", "Alpaca", "Angel One (Indian Market)", "Alpha Vantage", "Polygon.io", "Finnhub", "Yahoo Finance"],
        index=6
    )
